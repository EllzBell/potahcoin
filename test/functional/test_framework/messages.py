...
"""Potahcoin test framework primitive and message structures

CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
    data structures that should map to corresponding structures in
    potahcoin/primitives
...
# Objects that map to potahcoind objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/potahcoin/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in potahcoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            self.scrypt256 = uint256_from_str(potahcoin_scrypt.getPoWHash(r))
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in potahcoind indicates these should be deserialized as blocks
