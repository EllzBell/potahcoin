...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, ConnectionType conn_type)
{
    ...
    CNode* pnode = new CNode(id, nLocalServices, GetBestHeight(), hSocket, addrConnect, CalculateKsproutdNetGroup(addrConnect), nonce, addr_bind, pszDest ? pszDest : "", conn_type);
    pnode->AddRef();
...
void CNode::copyStats(CNodeStats &stats, const std::vector<bool> &m_asmap)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Potahcoin users should be well used to small numbers with many decimal places by now :)
...
static bool CompareNetGroupKsproutd(const NodeEvictionCandidate &a, const NodeEvictionCandidate &b) {
    return a.nKsproutdNetGroup < b.nKsproutdNetGroup;
}
...
bool CConnman::AttemptToEvictConnection()
{
    std::vector<NodeEvictionCandidate> vEvictionCandidates;
    {
        ...
        for (const CNode* node : vNodes) {
            ...
            NodeEvictionCandidate candidate = {node->GetId(), node->nTimeConnected, node->nMinPingUsecTime,
                                               ...
                                               peer_relay_txes, peer_filter_not_null, node->nKsproutdNetGroup,
    ...
    EraseLastKElements(vEvictionCandidates, CompareNetGroupKsproutd, 4);
    ...
    for (const NodeEvictionCandidate &node : vEvictionCandidates) {
        std::vector<NodeEvictionCandidate> &group = mapNetGroupNodes[node.nKsproutdNetGroup];
        ...
        if (group.size() > nMostConnections || (group.size() == nMostConnections && grouptime > nMostConnectionsTime)) {
            ...
            naMostConnections = node.nKsproutdNetGroup;
        }
    }
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    CNode* pnode = new CNode(id, nodeServices, GetBestHeight(), hSocket, addr, CalculateKsproutdNetGroup(addr), nonce, addr_bind, "", ConnectionType::INBOUND, inbound_onion);
    pnode->AddRef();
...
CNode::CNode(NodeId idIn, ServiceFlags nLocalServicesIn, int nMyStartingHeightIn, SOCKET hSocketIn, const CAddress& addrIn, uint64_t nKsproutdNetGroupIn, uint64_t nLocalHostNonceIn, const CAddress& addrBindIn, const std::string& addrNameIn, ConnectionType conn_type_in, bool inbound_onion)
    : nTimeConnected(GetSystemTimeInSeconds()),
    addr(addrIn),
    addrBind(addrBindIn),
    nKsproutdNetGroup(nKsproutdNetGroupIn),
...
uint64_t CConnman::CalculateKsproutdNetGroup(const CAddress& ad) const
{
