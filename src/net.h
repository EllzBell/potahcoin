...
#ifndef POTAHCOIN_NET_H
#define POTAHCOIN_NET_H
...
enum class ConnectionType {
     ...
     * Note that in the literature ("Eclipse Attacks on Potahcoin’s Peer-to-Peer Network")
...
private:
    ...
    uint64_t CalculateKsproutdNetGroup(const CAddress& ad) const;
...
public:
    ...
    const uint64_t nKsproutdNetGroup;
...
public:
    ...
    CNode(NodeId id, ServiceFlags nLocalServicesIn, int nMyStartingHeightIn, SOCKET hSocketIn, const CAddress &addrIn, uint64_t nKsproutdNetGroupIn, uint64_t nLocalHostNonceIn, const CAddress &addrBindIn, const std::string &addrNameIn, ConnectionType conn_type_in, bool inbound_onion = false);
    ~CNode();
