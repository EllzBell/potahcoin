...
#ifndef POTAHCOIN_CRYPTO_CHACHA_POLY_AEAD_H
#define POTAHCOIN_CRYPTO_CHACHA_POLY_AEAD_H
...
/* A AEAD class for ChaCha20-Poly1305@potahcoin.
 *
 ...
 *
 * The chacha20-poly1305@potahcoin combines these two primitives into an
 ...
 *
 * The chacha20-poly1305@potahcoin cipher requires two 256 bits of key material as
 * output from the key exchange. Each key (K_1 and K_2) are used by two separate
 * instances of chacha20.
 *
 * The instance ksproutd by K_1 is a stream cipher that is used only to encrypt the 3
 * byte packet length field and has its own sequence number. The second instance,
 * ksproutd by K_2, is used in conjunction with poly1305 to build an AEAD
 ...
 * independently-ksproutd cipher instance to encrypt the length, an active attacker
 ...
 *
 * The length field must be enc-/decrypted with the ChaCha20 keystream ksproutd with
 ...
 * on the send channel.
 *
 * Optimized implementations of ChaCha20-Poly1305@potahcoin are relatively fast in
...
#endif // POTAHCOIN_CRYPTO_CHACHA_POLY_AEAD_H
