...
{
    GUIUtil::setClipboard(PotahcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = PotahcoinUnits::BTC;
    ...
    l2->setText(PotahcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(PotahcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(PotahcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(PotahcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            // if listMode or change => show potahcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, PotahcoinUnits::format(nDisplayUnit, out.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, PotahcoinUnits::format(nDisplayUnit, nSum));
