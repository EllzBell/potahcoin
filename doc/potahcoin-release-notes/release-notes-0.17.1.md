Potahcoin Core version 0.17.1 is now available from:

  <https://download.potahcoin.org/potahcoin-0.17.1/>
...
When potahcoin is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `potahcoin.conf` file, not inside included files or from
  ...
  as potahcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the potahcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=potahcoin
    test.uacomment=potahcoin-testnet
...
be used in V0.17 by starting potahcoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Potahcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/potahcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Potahcoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://potahcointalk.org/?topic=279249) transactions.

For backend RPC convenience, the Potahcoin devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed Potahcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Potahcoin transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When potahcoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If potahcoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart potahcoind with
...
- When running potahcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If potahcoind
...
- #11962 `0a01843` add seed.potahcoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI potahcoin:// (krab)
...
- #13528 `b877c39` Move PotahcoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libpotahcoin-util to libpotahcoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for potahcoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling potahcoin-qt (TheCharlatan)
...
- #14698 `ec71f06` Add potahcoin-tx.exe into Windows installer (ken2812221)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use PotahcoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to potahcoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use PotahcoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use PotahcoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use PotahcoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for PotahcoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_potahcoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.potahcoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_potahcoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in potahcoind systemd service file (Flowdalic)
- #12652 `55f490a` potahcoin-cli: Provide a better error message when potahcoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to potahcoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for potahcoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` potahcoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix potahcoin-cli --version (Empact)
...
- #12877 `2b54155` Use potahcoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update potahcoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust potahcoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed potahcoin-cli -help output for help2man (hebasto)
