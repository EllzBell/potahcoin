...
The Potahcoin Core daemon can be configured to act as a trusted "border
router", implementing the potahcoin wire protocol and relay, making
...
The ZeroMQ feature in Potahcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building potahcoind:
...
For instance:
    ...
    $ potahcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/potahcoind.tx.raw \
...
These options can also be provided in potahcoin.conf.
...
From the perspective of potahcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into potahcoind directly. Furthermore, no information is
...
using. Potahcoind appends an up-counting sequence number to each
