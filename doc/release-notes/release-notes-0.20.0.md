...
Potahcoin Core version 0.20.0 is now available from:

  <https://potahcoincore.org/bin/potahcoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Potahcoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Potahcoin Core on
unsupported systems.

From Potahcoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Potahcoin Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `POTAHCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Potahcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Potahcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Potahcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Potahcoin Core. Potahcoin Core logs debug messages
...
- OpenSSL is no longer used by Potahcoin Core.  (#17265)

- BIP70 support has been fully removed from Potahcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/potahcoin/potahcoin/issues/16599),
  [PR #16702](https://github.com/potahcoin/potahcoin/pull/16702), and the
  `potahcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Potahcoin Core starts are now
...
- The "Start Potahcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://potahcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Potahcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Potahcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/potahcoin-core/HWI) for
  signing.  Future versions of Potahcoin Core should support a GUI option
...
- Potahcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.potahcoincore.org.  (#17596)
...
- The `potahcoin-cli` utility used with the `-getinfo` parameter now
...
- The `potahcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `potahcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_potahcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBPOTAHCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_potahcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBPOTAHCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for potahcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate potahcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest potahcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_potahcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (potahcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone potahcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Potahcoin Core files (hebasto)
...
- #17119 Fix broken potahcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from potahcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.potahcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to potahcoin-wallet (hel-o)
...
- #16115 On potahcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from potahcoin-cli -getinfo (malevolent)
